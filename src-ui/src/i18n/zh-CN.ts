import { editorFallback, type I18nKey } from './en';

export const zhCN = {
  ...editorFallback,
  'app.title': 'Sinos CLI',
  'explorer.tab.workspace': '工作区',
  'explorer.tab.history': '会话记录',
  'explorer.workspace.select-dir': '点击选择工作目录',
  'explorer.loading': '正在加载文件…',
  'explorer.empty': '此文件夹为空',
  'explorer.root_error': '无法读取工作区目录',
  'explorer.directory_error': '无法读取此文件夹',
  'explorer.delete_confirm_file': '确定永久删除“{name}”吗？此操作无法撤销。\n\n{path}',
  'explorer.delete_confirm_dir': '确定永久删除文件夹“{name}”及其中的全部内容吗？此操作无法撤销。\n\n{path}',
  'explorer.error_outside_workspace': '已阻止操作：路径位于当前工作区之外',
  'explorer.error_workspace_root': '不能在这里删除或修改工作区根目录',
  'explorer.error_invalid_name': '新名称无效；这里只能填写单个文件名或文件夹名',
  'explorer.error_paste_same_path': '所选项目已经位于此文件夹中',
  'explorer.error_paste_self': '不能把文件夹复制或移动到自身内部',
  'explorer.error_destination_exists': '目标位置已存在同名文件或文件夹',
  'explorer.error_symlink': '文件浏览器不支持修改符号链接或 Windows 重解析点',
  'explorer.error_workspace_unavailable': '工作区已不可用',
  'explorer.error_target_unavailable': '粘贴目标目录已不可用',
  'explorer.error_path_unavailable': '所选路径已不可用',
  'explorer.error_delete_failed': '无法删除所选项目',
  'explorer.error_rename_failed': '无法重命名所选项目',
  'explorer.error_move_failed': '无法移动所选项目',
  'explorer.error_copy_failed': '无法复制所选项目',
  'explorer.error_copy_rollback_failed': '复制失败，且无法清理目标位置留下的不完整内容',
  'explorer.error_generic': '文件操作失败',
  'image_viewer.title': '图片预览',
  'image_viewer.zoom_out': '缩小',
  'image_viewer.zoom_in': '放大',
  'image_viewer.rotate': '旋转',
  'image_viewer.close': '关闭',
  'image_viewer.previous': '上一张图片',
  'image_viewer.next': '下一张图片',
  'image_viewer.load_failed': '无法加载此图片',
  'image_viewer.loading': '正在加载图片…',
  'image_viewer.hint': '滚轮缩放 · ← → 切换 · R 旋转 · Esc 关闭',

  // Context Menu
  'menu.copy_abs': '复制绝对路径',
  'menu.copy_rel': '复制相对路径',
  'menu.copy_ref': '复制为 @reference',
  'menu.cut': '剪切',
  'menu.copy': '复制',
  'menu.paste': '粘贴',
  'menu.select_all': '全选',
  'menu.rename': '重命名',
  'menu.delete': '删除',
  'menu.show_in_folder': '在文件管理器中显示',
  'menu.open': '打开',
  'menu.pin': '置顶',
  'menu.unpin': '取消置顶',
  'menu.copy_session_id': '复制会话ID',
  'menu.copy_resume_command': '复制恢复命令',
  'menu.copy_full_path': '复制完整路径',


  // Tools
  'tool.terminal': '终端',
  'tool.remote': '远程终端',
  'library.agent_tools': 'Agent 工具',
  'sentinel.protocol': '哨兵协议',
  'tool.two_split': '独立二屏',
  'tool.three_split': '独立三屏',
  'tool.four_split': '独立四屏',
  'tool_config.command': '启动命令',
  'tool_config.extra_args': '额外参数',
  'tool_config.default_cwd': '启动目录',
  'tool_config.history_path': '历史对话目录',
  'tool_config.reset': '重置',
  'tool_config.cancel': '取消',
  'tool_config.save': '保存',

  // Remote Terminal
  'remote.title': '远程终端',
  'remote.host': '服务器地址',
  'remote.host_placeholder': '例如 192.168.1.100',
  'remote.username': '用户名',
  'remote.password': '密码',
  'remote.connect': '连接',
  'remote.connecting': '连接中...',
  'remote.connect_failed': '连接失败',

  'tab.new': '选择工具',


  // Task Board
  'task.notes_placeholder': '添加备注...',
  'task.section.working': '进行中',
  'task.section.todo': '待办',
  'task.section.done': '已完成',
  'task.greeting.morning': '早，今天想做点什么',
  'task.greeting.afternoon': '下午好，还有什么要做的？',
  'task.greeting.evening': '晚上好，想干点什么大事？',
  'task.tab.tasks': '任务列表',
  'task.tab.changes': '修改记录',
  'changes.empty': '暂无修改。',
  'launchpad.open_folder': '打开文件夹',
  'launchpad.detect_help_trigger': "识别不到?",
  'launchpad.detect_help_tip': "若你曾经用过英文开发者制作的 AI 终端工具,可能把含中日韩等其他语言字符的 PATH 改成乱码,导致所有终端都识别不到。3种修复方案 ① 重装该 AI 工具 ② 手动修复工具 PATH ③ 让 AI 帮你修复 PATH",
  'launchpad.switch_model_trigger': "切换模型",
  'launchpad.switch_model_tip': "给所有的 AI 工具一键快速地切换模型,推荐使用 EchoBird,点击\"切换模型\"打开 Echobird.ai 下载安装即可。",
  'changes.clean': '没有改动 — 工作区干净。',
  'changes.no_git': '未安装 Git — 代码对比、分支等功能不可用。',
  'changes.not_repo': '当前文件夹不是 Git 仓库。',
  'changes.init_here': '在此初始化 Git',
  'changes.initializing': '正在初始化…',
  'changes.committed': '已提交',
  'changes.uncommitted': '未提交',
  'changes.untracked': '未跟踪',
  'diff.loading': '加载中…',
  'diff.error': '无法加载差异',
  'diff.no_changes': '没有改动',
  'diff.too_large': '文件较大,未渲染逐行差异',
  'diff.unchanged_lines': '⋯ {count} 行未改动',
  'task.default_title': '新任务',
  'task.search_sessions': '搜索历史对话...',
  'task.filter_agent': '按 Agent 筛选',
  'task.filter_project': '按项目筛选',
  'task.filter_all_agents': '全部 Agent',
  'task.no_matching_sessions': '没有符合当前筛选条件的会话',
  'task.filter_all_projects': '全部',
  'menu.no_recent': '没有任何近期会话',
  'task.messages': '{count} 条消息',

  // Actions

  // Time
  'time.just_now': '刚刚',
  'time.today': '今天',
  'time.yesterday': '昨天',
  'time.days_ago': '{days}天前',

  // Settings modal (titlebar gear)
  'settings.title': '设置',
  'settings.appearance': '外观',
  'settings.wallpaper': '壁纸',
  'settings.terminal': '终端',
  'settings.remote': '移动端',
  'settings.language': '语言',
  'settings.sound': '声音',
  'settings.sound.done': '任务完成提示音',
  'settings.sound.wait': '等待权限提示音',
  'settings.sound.on': '开',
  'settings.sound.off': '关',
  'settings.sound.preview': '试听',
  'interaction.permission': '需要确认权限',
  'interaction.question': '需要你的回答',
  'interaction.custom_placeholder': '输入你的补充说明…',
  'interaction.submit': '提交',
  'interaction.failed': '终端尚未就绪，请切换到终端界面回答。',
  'settings.feedback': 'BUG / 建议',
  'settings.feedback.desc': '遇到 BUG 或建议，点击下方提交你的问题：',
  'settings.wallpaper.pick': '选择图片或视频',
  'settings.wallpaper.clear': '移除壁纸',
  'settings.wallpaper.opacity': '透明度',
  'settings.terminal.scheme': '文字颜色',
  'settings.terminal.font': '字体',
  'settings.terminal.font.default': '默认(内置)',
  'settings.terminal.shell': '默认终端',
  'settings.terminal.shell.auto': '系统默认',
  'settings.terminal.shell.not_recommended': '不推荐',
  'settings.font.monospace': '等宽',
  'settings.font.other': '其他字体',
  'settings.font.search': '搜索字体…',
  'settings.gambit': '快捷键',
  'settings.send.title': '发送消息',
  'settings.send.newline': '换行',
  'settings.gambit.hotkey': '开关快捷键',
  // 顶部快捷开关显示模式(设置 → 快捷键)
  'settings.titlebar.toggle': '顶部快捷开关',
  'settings.titlebar.toggle.icon-hotkey': '图标 + 快捷键',
  'settings.titlebar.toggle.icon': '仅图标',
  'settings.titlebar.toggle.hidden': '隐藏',
  'settings.tasks': '任务',
  'settings.tasks.view': '任务形态',
  'task.view.list': '待办式',
  'task.view.note': '便签式',
  'task.view.list.sub': '紧凑清单，逐条勾选',
  'task.view.note.sub': '大便签，随手写、整段发给 AI',
  'task.view.prompt': '分类式',
  'task.view.prompt.sub': '按分类攒 AI 提示词，一键发送',
  'task.prompt.new_category': '新分类',
  'task.prompt.category_placeholder': '分类名（选已有或输入新的）',
  'task.prompt.body_placeholder': '写提示词，整段发给 AI…',
  'task.note_placeholder': '写点什么，整段发给 AI…',
  'task.welcome_note': `欢迎来到 Sinos CLI

这张就是「便签」——随手写，写完点右上角 ▶ 整段发给 AI，不用再挤小输入框。

上手三步：
1. 中间挑个工具（Claude / Codex / OpenCode…），选好文件夹就开跑
2. 左上三个圆点切优先级（红 / 黄 / 绿），自动排序；拖底边能把便签拉大拉小
3. 右上「修改记录」看 AI 改了哪些文件；设置里换主题、语言，或切回「待办式」

看完删掉这张，开始你的第一个任务吧。`,
  'task.show_guide': '查看使用攻略',

  // Theme Menu
  'theme.section.color': '配色',
  'theme.section.shape': '形态',
  'theme.section.icons': '图标风格',
  // 主题风格（在“配色”右侧切换白天/夜晚/跟随系统）
  'theme.mode.light': '明亮',
  'theme.mode.dark': '夜晚',
  'theme.mode.system': '跟随主题',
  'theme.color.light': '雾灰',
  'theme.color.dark': '拿铁',
  'theme.color.sakura': '樱花',
  'theme.color.lavender': '薰衣草雾',
  'theme.color.mint': '薄荷深海',
  'theme.color.obsidian': '石墨',
  'theme.color.moss': '苔藓',
  'theme.color.amber': '琥珀',
  'theme.color.teal': '葡萄紫',
  'theme.color.indigo': '靛蓝',
  'theme.color.glacier': '冰川蓝',
  'theme.color.slate': '玫瑰',

  // Gambit · 妙手
  'gambit.title': '妙手',
  'gambit.placeholder': '静心琢磨，再落子... ({send} 发送, {newline} 换行, 可粘贴图片, ←→ 翻历史)',
  'gambit.send_failed_hint': '请先打开活动会话',
  'gambit.send_empty_hint': '先输入内容或粘贴图片 (Ctrl+V)',
  'gambit.upload_image': '上传图片',
  'gambit.interrupt': '停止当前任务',


  // 贡献热力图（桌面 Launchpad 上的 6 张卡片上方）
  'conversation.reasoning': '思考过程',
  'conversation.thinking': '正在思考中…',
  'conversation.executing': '正在执行中…',
  'conversation.copied': '已复制',
  'conversation.copy_message': '复制消息',
  'conversation.navigation': '对话快速定位',
  'conversation.jump_to_turn': '跳转到第 {turn} 轮提问',
  'heatmap.title': '{sessions} 次会话、{messages} 条消息',
  'heatmap.title_empty': '故事还没开始 — 跟 AI 聊起来，点亮你的第一格',
  'heatmap.legend_less': '少',
  'heatmap.legend_more': '多',
  'heatmap.tooltip_some': '{date} · {count} 条消息',
  'heatmap.tooltip_one': '{date} · 1 条消息',
  'heatmap.tooltip_none': '{date} · 没有活动',

  // 应用内代码编辑器
  'menu.open_editor': '在编辑器中打开',
  'editor.loading': '正在加载编辑器…',
  'editor.reading': '正在读取文件…',
  'editor.retry': '重试',
  'editor.save': '保存',
  'editor.saving': '保存中…',
  'editor.reload': '重新加载',
  'editor.reload_confirm': '重新加载会丢弃当前未保存的修改，确定继续吗？',
  'editor.saved': '已保存',
  'editor.unsaved': '有未保存修改',
  'editor.external_changed': '磁盘文件已变化，未保存内容已保留',
  'editor.external_save_blocked': '文件已被外部程序修改，请重新加载后再保存',
  'editor.external_conflict': '文件已被外部程序修改，已停止保存以避免覆盖改动',
  'editor.binary': '二进制文件不能在编辑器中打开',
  'editor.encoding': '当前文件不是 UTF-8 编码，无法打开',
  'editor.too_large': '文件超过 5 MB 编辑上限',
  'editor.outside_workspace': '文件不在当前工作区内',
  'editor.read_failed': '文件读取失败',
  'editor.close_unsaved': '文件尚未保存，确定关闭吗？',
  'editor.close_error': '关闭错误提示',
  'editor.mixed_line_endings': '文件包含混合换行符，保存时将统一为 LF',
  'editor.file_unavailable': '文件已不可用',
  'editor.workspace_unavailable': '工作区已不可用',
  'editor.not_a_file': '所选路径不是文件',
  'editor.replace_failed': '保存时无法替换文件',
  'editor.permission_copy_failed': '保存时无法保留文件权限',
  'editor.parent_unavailable': '文件所在目录已不可用',
  'editor.temp_create_failed': '保存时无法创建临时文件',
  'editor.write_failed': '文件写入失败',
  'editor.flush_failed': '文件无法可靠写入磁盘',

  'gambit.pause': '暂停当前任务',
  'gambit.resume': '恢复当前任务',
  'gambit.send': '发送',
  'changes.tracking': '跟踪中',
  'changes.tracking_disable': '关闭此项目的 Git 修改记录跟踪',
  'changes.tracking_enable': '开启修改记录跟踪',
  'changes.tracking_disabled': '此项目已关闭 Git 修改记录跟踪。',
} as const satisfies Record<I18nKey, string>;
