...
export function PhoneWorkspace({ client }: { client: RemoteClient }) {
  ...
  return <div className="remote-app phone-workspace" data-mobile-theme={theme}>
    <div className="phone-shell" inert={panel === 'sessions'}>
      ...
      {!online && <div className="phone-network" role="status"><span className="status-dot" /><span>{connection === 'connecting' ? '正在连接电脑…' : connection === 'unauthorized' ? '配对已失效，请重新配对' : connection === 'replaced' ? '已在另一个页面连接' : '电脑暂时离线，正在重新连接…'}</span><button onClick={() => { if (connection === 'unauthorized' && client instanceof RelayClient) { client.dispose(); forgetDevice(); location.reload(); } else { if (connection === 'replaced' && client instanceof RelayClient) client.resumeHere(); refresh(); } }}>{connection === 'unauthorized' && client instanceof RelayClient ? '重新配对' : connection === 'replaced' ? '在此页面连接' : '重试'}</button></div>}
    ...
    {panel === 'settings' && <Sheet title="连接与显示" onClose={() => setPanel(null)}><div className="connection-card"><Monitor size={25} /><div><strong>{state.device_name || '我的电脑'}</strong><p>{online ? '已连接' : '等待连接'}</p></div></div><dl className="connection-details"><div><dt>连接方式</dt><dd>{client instanceof RelayClient ? 'Cloudflare · 端到端加密' : '本地网络'}</dd></div><div><dt>活动会话</dt><dd>{state.sessions.length}</dd></div></dl><div className="setting-row"><span>外观</span><div className="segmented-control"><button aria-pressed={theme === 'light'} onClick={() => setTheme('light')}><Sun size={16} />浅色</button><button aria-pressed={theme === 'dark'} onClick={() => setTheme('dark')}><Moon size={16} />深色</button></div></div><p className="sheet-footnote">保持电脑上的 Sinos 运行。可通过浏览器菜单添加到主屏幕，以独立窗口打开。</p>{client instanceof RelayClient && <button className="danger-button full-width" onClick={() => { if (!confirmForget) { setConfirmForget(true); return; } client.dispose(); forgetDevice(); location.reload(); }}>{confirmForget ? '确认移除本机配对记录' : '断开并忘记这台电脑'}</button>}<a className="license-link" href="/third-party/EnsoCode-LICENSE.txt" target="_blank" rel="noreferrer">开源许可 · EnsoCode</a></Sheet>}
  </div>;
}
