...
export function PhoneWorkspace({ client }: { client: RemoteClient }) {
  ...
  const [panel, setPanel] = useState<'sessions' | 'new' | 'tools' | 'settings' | 'share' | null>(null);
  ...
  const setCwd = useCallback((cwd: string) => { if (selected) setPaths(values => values[selected.id] === cwd ? values : { ...values, [selected.id]: cwd }); }, [selected]);
  const projectPath = (session: (typeof state.sessions)[number]) => paths[session.id] || session.cwd;
  ...
  return <div className="remote-app phone-workspace" data-mobile-theme={theme} style={appearance.style}>
    <div className="phone-shell" inert={panel === 'sessions'}>
      <header className="phone-header">
        ...
        <button className="phone-heading" onClick={() => setPanel('sessions')}><strong>{selected ? titles[selected.id] || name(selected.tool) : 'Sinos'}</strong><span><span className="phone-project-path" title={selected ? projectPath(selected) : undefined}>{selected ? projectPath(selected) || '工作区' : '你的移动工作空间'}</span><i className={`status-dot ${online ? 'is-online' : 'is-pending'}`} /></span></button>
        <button className="icon-button" aria-label="新建会话" disabled={!online} onClick={() => setPanel('new')}><SquarePen size={19} /></button>
        {selected && client instanceof RelayClient && state.capabilities?.includes('temporary_shares_v1') && <button className="icon-button" aria-label="分享当前会话" disabled={!online} onClick={() => setPanel('share')}><Share2 size={18} /></button>}
      ...
      <main className="phone-main">
        {selected ? <div className="session-workspace" key={selected.id}>
          <section className="mobile-tab-panel" hidden={surface !== 'chat'} aria-label="对话"><ChatView client={client} session={selected} online={online} capabilities={state.capabilities} toolName={name(selected.tool)} onTitle={setTitle} onCwd={setCwd} insert={insert} onInserted={inserted} /></section>
    ...
    {panel === 'share' && selected && client instanceof RelayClient && <ShareSheet client={client} sessionId={selected.id} onClose={() => setPanel(null)} />}
    {panel === 'sessions' && <div className="drawer-backdrop" onClick={() => setPanel(null)}><aside className="session-drawer" role="dialog" aria-modal="true" aria-label="会话列表" onClick={e => e.stopPropagation()}><div className="drawer-brand"><Coffee size={22} /><strong>Sinos</strong><button className="icon-button" aria-label="关闭会话列表" autoFocus onClick={() => setPanel(null)}><X size={18} /></button></div><button className="drawer-new" disabled={!online} onClick={() => setPanel('new')}><SquarePen size={17} />新建会话</button><div className="drawer-sessions">{[...new Set(state.sessions.map(projectPath))].map(cwd => <div className="drawer-group" key={cwd}><p><Folder size={13} />{projectName(cwd)}</p>{state.sessions.filter(s => projectPath(s) === cwd).map(session => <button key={session.id} className={`drawer-session ${session.id === selected?.id ? 'active' : ''}`} onClick={() => choose(session.id)}><MessageSquare size={15} /><span>{titles[session.id] || name(session.tool)}<small>{session.paused ? '已暂停' : session.activity?.state === 'working' ? '正在生成' : session.activity?.state === 'waiting' ? '等待回答' : session.activity?.state === 'idle' ? '就绪' : session.activity?.state === 'failed' ? '本轮已停止' : '会话已连接'}{session.queued_count ? ' · 待发送 ' + session.queued_count : ''} · {session.id.slice(0, 6)}</small></span>{session.id === selected?.id && <Check size={14} />}</button>)}</div>)}{!state.sessions.length && <p className="drawer-empty">还没有会话</p>}</div><button className="drawer-device" onClick={() => setPanel('settings')}><Monitor size={18} /><span>{state.device_name || '我的电脑'}<small>{online ? '已连接' : '等待连接'}</small></span><Settings2 size={17} /></button></aside></div>}
    {panel === 'new' && <LaunchSheet client={client} tools={tools} sessions={state.sessions} cwd={state.default_cwd || ''} onClose={() => setPanel(null)} onLaunched={id => { choose(id); setPendingLaunch(id); refresh(); }} />}
