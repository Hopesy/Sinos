...
export function ConversationQuestion({ question, disabled, unsupported, answering, answered, disconnected, readOnly = false, onAnswer }: {
  ...
  const blocked = readOnly || disabled || unsupported || answering || answered;
  ...
  return <section className="conversation-question" aria-label={question.kind === 'text' ? '需要你的回答' : '需要你的确认'}>
    ...
    {!readOnly && question.kind !== 'multi' && question.cursor !== undefined && <p>点击选项即可确认</p>}
    {readOnly ? <p role="status">等待分享者操作</p> : unsupported ? <p role="status">电脑端需更新后才能在这里回答。</p> : answering ? <p role="status">正在提交你的选择…</p> : answered ? <p role="status">已回复，等待继续…</p> : disconnected && <p role="status">连接恢复后即可回答</p>}
  </section>;
}
