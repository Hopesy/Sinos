...
export function buildPairUri(invite: PairInvite): string {
  ...
  return `coffee://pair?relay=${encodeURIComponent(invite.relay)}&pk=${pk}`;
}
...
/** 解析配对码：兼容 coffee:// 自定义 scheme 与 https 链接（片段或查询串） */
export function parsePairUri(uri: string): PairInvite {
  const text = uri.trim();
  const custom = /^coffee:\/\/pair\?(.*)$/.exec(text);
  ...
  throw new Error('not an coffee pair uri');
}
