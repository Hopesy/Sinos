...
export function LaunchSheet({ client, tools, sessions, cwd, onClose, onLaunched }: { client: RemoteClient; tools: RemoteTool[]; sessions: RemoteSession[]; cwd: string; onClose: () => void; onLaunched: (id: string) => void }) {
  ...
  const [folder, setFolder] = useState(cwd || storageRead('last-project'));
  ...
  const launch = async () => {
    ...
    try { const result = await client.launch(selected.id, folder.trim()); storageWrite('last-project', folder.trim()); onLaunched(result.session_id); }
  ...
  return <Sheet title="开启新会话" onClose={() => { if (!busy) onClose(); }}>
    ...
    <label className="sheet-field"><span>电脑上的项目目录 <small>可选</small></span><input value={folder} onChange={(e) => setFolder(e.target.value)} placeholder="留空使用桌面端当前目录" autoCapitalize="off" autoCorrect="off" spellCheck={false} /></label>
