...
describe('register', () => {
  ...
  test('mirrors visible streaming chunks without changing the engine or leaking thinking', async ($, on) => {
    const sent = [];
    mock.env(on, { SINOS_CLAUDE_BRIDGE_URL: 'http://127.0.0.1:12345/events', SINOS_CLAUDE_BRIDGE_TOKEN: 'test-token' });
    on('session.start', ($, e) => ({ cwd: e.cwd }));
    on('session.id', () => ({ value: 'session-1' }));
    ...
    on('turn.start', ($, e) => ({ turnId: e.turnId }));
    const chunks = [{ kind: 'text', index: 0, text: '1. **First**\n' }, { kind: 'thinking', index: 1, text: 'private' }, { kind: 'text', index: 2, text: '2. Second\n' }];
