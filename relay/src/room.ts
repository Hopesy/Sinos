...
export class PairRoom extends DurableObject<Env> {
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (path === '/cancel') return this.onCancel(req);
  ...
  private async onCancel(req: Request): Promise<Response> {
    const { requestToken } = await req.json() as { requestToken?: string };
    if (typeof requestToken !== 'string' || !/^[A-Za-z0-9_-]{32,128}$/.test(requestToken)) return json({ error: 'invalid request token' }, 400);
    const state = await this.ctx.storage.get<PairState>('state');
    if (state && state.requestToken !== requestToken) return json({ error: 'invitation owner mismatch' }, 403);
    // Also undo a claim racing with cancellation before the desktop accepts it.
    await this.revoke();
    return json({ ok: true });
  }
